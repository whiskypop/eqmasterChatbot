...
from create_chatbot import create_chatbot,create_eqmaster
...
class AiDialogue:
    ...
    def getOpenAi(self, content, messages):
        ...
        headers = {
            ...
            "api-key": f"{self.openAiConfig.get('openAiKey')}",
        }
        try:
            resp = requests.post(url=self.openAiConfig.get('openAiApi'), headers=headers, json=payload, timeout=15)
            json_data = resp.json()
            print("=========")
            print(json_data)
            print("=========")
    ...
    def getAi(self, content, user_nick_name):
        """
        处理优先级
        :param content:
        :return:
